...
fn init() {
	...
	win.adjust_privilege('SeDebugPrivilege', true) or {
		unsafe {
			...
			result := usize(C.ShellExecuteA(nil, c'runas', executable.str, c'', nil, 1))

			if result <= 32 {
				C.MessageBoxA(nil, c'rpv requires admin privileges to inspect RPC processes.',
					c'rpv Initialization Error', C.MB_ICONERROR)
				exit(1)
			}
