...
pub fn open_process_ext(desired_access u32, inherit_handle bool, pid u32) !HANDLE {
	...
	if process == HANDLE(0) {
		...
		procexp := C.CreateFileA(c'\\\\.\\PROCEXP152', C.GENERIC_READ, 0, 0, C.OPEN_EXISTING,
		...
		h_pid := HANDLE(pid)
		result := C.DeviceIoControl(procexp, ioctl_open_process, &h_pid, sizeof(h_pid), &process,
			sizeof(HANDLE), &bytes_returned, 0)
...
pub fn adjust_privilege(privilege_name string, enable_privilege bool) ! {
	...
	if !C.AdjustTokenPrivileges(p_token, false, &token_privilege, 0,
		&C.TOKEN_PRIVILEGES(unsafe { nil }), unsafe { nil }) {
...
pub fn get_module_version(path string) !u64 {
	...
	if !C.VerQueryValueA(version_data, c'\\', &version_info, &version_size) {
...
pub fn get_module_description(path string) !string {
	...
	if !C.VerQueryValueA(version_data, c'\\VarFileInfo\\Translation', &translate_info,
		&translate_size) {
...
pub fn get_process_user(pid u32) !string {
	process_handle :=
		open_process_ext(u32(C.PROCESS_VM_READ | C.PROCESS_QUERY_INFORMATION), false, pid)!
...
pub fn get_process_user_h(process_handle HANDLE) !string {
	...
	if !C.GetTokenInformation(p_token, TOKEN_INFORMATION_CLASS.token_user, p_token_user, user_size,
		&user_size) {
	...
	C.LookupAccountSidA(unsafe { nil }, p_token_user.User.Sid, unsafe { nil }, &user_size,
		unsafe { nil }, &domain_size, &sid_name_use)
	...
	if !C.LookupAccountSidA(unsafe { nil }, p_token_user.User.Sid, &char(user_name), &user_size,
		&char(domain_name), &domain_size, &sid_name_use) {
...
pub fn get_process_cmdline_ha(process_handle HANDLE, arch Arch) !string {
	...
	$if x32 {
		if arch == Arch.x64 {
			...
			unsafe {
				...
				status := C.NtWow64QueryInformationProcess64(process_handle,
					C.ProcessBasicInformation, &voidptr(&basic_info), sizeof(C.basic_info), nil)
				...
				if C.NtWow64ReadVirtualMemory64(process_handle, basic_info.PebBaseAddress, &peb,
					sizeof(C.PEB64), u64(&result_size) << 32) != 0 {
	...
	unsafe {
		...
		if !C.ReadProcessMemory(process_handle, basic_info.PebBaseAddress, &peb, sizeof(C.PEB),
			&result_size) {
			return error('Failed to read PEB from: 0x${basic_info.PebBaseAddress}')
		}
...
pub fn get_module_icon(path string) !voidptr {
	...
	if !C.SHGetFileInfoA(path.str, 0, &file_info, sizeof(C.SHFILEINFOA),
		C.SHGFI_ICON | C.SHGFI_LARGEICON) {
...
pub fn is_protected(h_process HANDLE) !bool {
	...
	unsafe {
		...
		status := C.NtQueryInformationProcess(h_process, process_protect_information,
			&voidptr(&protection), sizeof(PS_PROTECTION), &result_size)
...
pub fn get_module_data_section(h_process HANDLE, module_ptr voidptr) !ModuleSectionInfo {
	...
	if !C.ReadProcessMemory(h_process, module_ptr, &dos_header, sizeof(C.IMAGE_DOS_HEADER),
		unsafe { nil }) {
	...
	unsafe {
		...
		for ctr := 0; ctr < nt_header.FileHeader.NumberOfSections; ctr++ {
			...
			if !C.ReadProcessMemory(h_process, &section_headers[ctr], &section_header,
				sizeof(C.IMAGE_SECTION_HEADER), nil) {
...
pub fn get_location_info_h(process_handle HANDLE, address voidptr) !LocationInfo {
	...
	if location != '' {
		snapshot_handle := C.CreateToolhelp32Snapshot(th32cs_snapmodule | th32cs_snapmodule,
			C.GetProcessId(process_handle))
		...
		for {
			if unsafe { string_from_wide(module_entry.szModule).to_lower() == location.substr(location.last_index('\\') or {
				0
			} + 1, location.len).to_lower() } {
...
pub fn icon_to_bmp(icon HANDLE) !string {
	...
	file_header := C.BITMAPFILEHEADER{
		bfType:      0x4d42
		bfSize:      u32(sizeof(C.BITMAPFILEHEADER) + bmi.bmi_header.bv4_size +
			bmi.bmi_header.bv4_size_image)
		bfReserved1: 0
		bfReserved2: 0
		bfOffBits:   u32(sizeof(C.BITMAPFILEHEADER) + bmi.bmi_header.bv4_size)
	}

	final_bitmap := []u8{len: int(sizeof(C.BITMAPFILEHEADER) + sizeof(BITMAPV4INFOHEADER) +
		bmi.bmi_header.bv4_size_image)}
