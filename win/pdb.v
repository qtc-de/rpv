...
pub fn get_module_pdb_info(process_handle HANDLE, module_base voidptr) !CV_INFO_PDB70 {
	...
	if !C.ReadProcessMemory(process_handle, module_base, &dos_header, sizeof(dos_header),
		unsafe { nil }) {
	...
	unsafe {
		...
		if !C.ReadProcessMemory(process_handle, &u8(module_base) + u32(dos_header.e_lfanew),
			&nt_headers, sizeof(nt_headers), nil) {
		...
		if !C.ReadProcessMemory(process_handle, debug_dir, &debug_directory,
			sizeof(debug_directory), nil) {
			return error('Unable to read IMAGE_DEBUG_DIR.')
		}

		if !C.ReadProcessMemory(process_handle, &u8(module_base) +
			u32(debug_directory.AddressOfRawData), &pdb_info, sizeof(pdb_info), nil) {
